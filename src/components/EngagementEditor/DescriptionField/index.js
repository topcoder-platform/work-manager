...
import $ from 'jquery'
import { FILE_PICKER_API_KEY } from '../../../config/constants'
...
class DescriptionField extends Component {
  ...
  updateDescription () {
    const { onUpdateDescription } = this.props
    onUpdateDescription(this.currentValue)
  }

  onChange (value) {
    this.isChanged = true
    this.currentValue = value
    this.updateDescriptionThrottled(value)
  }

  getInitValue () {
    const { engagement, readOnly } = this.props
    if (!readOnly) {
  ...
  componentDidMount () {
    ...
    if (!readOnly) {
      ...
      this.setState({
        readOnlyDescription: engagement.description || ''
      })
    }
  }
  ...
  customUploadImage (file) {
    ...
    const onerror = (event) => {
      onErrorSup(this.easyMDE.options.errorMessages.importError)
    }
  ...
  render () {
    ...
    const { readOnlyDescription } = this.state
