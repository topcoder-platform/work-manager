...
const ReviewTypeField = ({ reviewers, challenge, onUpdateOthers, onUpdateSelect }) => {
  ...
  const communityOption = (disabled) => (
    <div className={styles.tcRadioButton}>
      <input
        ...
        checked={isCommunity}
        disabled={disabled}
        onChange={e => e.target.checked && onUpdateOthers({
          field: 'reviewType',
  ...
  return (
    <div>
        ...
        <div className={cn(styles.field, styles.col2)}>
          <div className={styles.subGroup}>
            <div className={styles.subRow}>
              ...
              {!isDesignChallenge && !isTask && communityOption()}
            </div>
            <div className={styles.subRow}>
              {isQAChallenge && (
      ...
      {challenge.submitTriggered && isInternal && !challenge.reviewer && (
        <div className={cn(styles.field, styles.row, styles.error)}>
          Select a reviewer
        </div>
      )}
    </div>
